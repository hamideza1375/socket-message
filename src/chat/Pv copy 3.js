...
const Pv = (p) => {
  ...
  useFocusEffect(useCallback(() => {
    ...
    // if(p.to) p.navigation.setParams({ to: p.to });

    if (!p.to) p.navigation.navigate('Chat')

      (async () => {
        await p.localStorage.setItem(p.route.params.item.userId, JSON.stringify(p.route.params.item))
      })()
